...
const [search, setsearch] = useState("")
   const handleChange = (event) => {
        ...
        let filterd = Contacts.filter(item => (item.lastname.toLowerCase().startsWith(search) || item.phone.startsWith(search) ||item.name.toLowerCase().startsWith(search)))
        return (
            <>
                <div id="navbar">
                    <div className="search-div">
                        <i id="searchIcon" className="fas fa-search"></i>
                        <input type="text"placeholder="search" name="search" className="searchInput" onChange={handleChange} />
                    </div>
